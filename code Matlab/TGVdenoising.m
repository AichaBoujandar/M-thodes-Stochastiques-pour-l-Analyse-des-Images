function x = TGVdenoising(y,theta1,theta2,sig,Nbiter)
	...
	prox_tau_fx = @(x) ((sig^2)*x+tau*y)/(sig^2+tau+2*tau*epsilon*(sig^2));
	...
	for iter = 1:Nbiter
		...
		u = prox_sigma_g_conj(u2+tau_prime*opJ(opD(2*x-x2)-(2*r-r2)));
		x2 = x2+rho*(x-x2);
		r2 = r2+rho*(r-r2);
		u2 = u2+rho*(u-u2);
		if mod(iter,25)==0
			primalcost = norm(x-y,'fro')^2/(2*(sig^2))+...
				theta1*sum(sum(sqrt(sum(r.^2,3))))+...
				theta2*sum(sum(sqrt(sum(opJ(opD(x)-r).^2,3))));		
			fprintf('nb iter:%4d  %f  \n',iter,primalcost);
			figure(6);
			imshow(x);
		end
	end
